...
export const StorefrontView = ({ slug, isCatalog = false }: { slug: string, isCatalog?: boolean }) => {
  ...
  const [showDescri├º├úo, setShowDescri├º├úo] = useState(false);
  ...
  const handleCepLookup = async (cep: string) => {
    ...
    try {
      ...
      if (data.erro) {
        showNotification('CEP n├úo encontrado', 'error');
        return;
      }
  ...
  const calculateShipping = async (cep: string) => {
    ...
    try {
      ...
      const formattedOptions = options.map((opt: any) => ({
        ...
        name: opt.name,
        price: opt.price,
        days: `${opt.days} dias ├║teis`,
        icon: opt.company?.toLowerCase().includes('correios') ? '­ƒôª' : '­ƒÜÜ'
      }));
      ...
      showNotification('N├úo foi poss├¡vel calcular o frete para este CEP. O servi├ºo pode estar indispon├¡vel.', 'error');
    } finally {
  ...
  const footerContent: Record<string, string> = {
    "Sobre N├│s": `Bem-vindo ├á ${store?.name || 'nossa loja'}. Somos especialistas em oferecer os melhores produtos com qualidade garantida e atendimento humanizado. Nossa miss├úo ├® facilitar sua jornada de compra com seguran├ºa e agilidade.`,
    "Seguran├ºa": "Sua seguran├ºa ├® nossa prioridade. Utilizamos criptografia SSL de ponta a ponta e parceiros de pagamento certificados para garantir que seus dados nunca sejam compartilhados ou acessados por terceiros.",
    "Termos de Uso": "Ao utilizar nossa loja, voc├¬ concorda com nossas diretrizes de navega├º├úo, prezando pela ├®tica e bom uso das ferramentas. Reservamo-nos o direito de atualizar pre├ºos e estoques conforme a disponibilidade.",
    "Privacidade": "Respeitamos sua privacidade. Seus dados cadastrais s├úo utilizados exclusivamente para o processamento de pedidos e melhoria da sua experi├¬ncia de compra, seguindo rigorosamente a LGPD.",
    "Como Comprar": "Para comprar, basta escolher seus produtos, adicionar ao carrinho e clicar em 'Finalizar'. Voc├¬ ser├í direcionado para o checkout onde poder├í escolher a melhor forma de pagamento e entrega.",
    "Prazos e Entregas": "Nossos prazos variam de acordo com sua regi├úo. Ap├│s a confirma├º├úo do pagamento, seu pedido ├® processado em at├® 24h ├║teis e o c├│digo de rastreamento enviado via e-mail.",
    "Formas de Pagamento": "Aceitamos PIX com aprova├º├úo imediata e Cart├Áes de Cr├®dito (Visa, Master, Elo, etc.) com parcelamento em at├® 12x, processados com seguran├ºa.",
    "Perguntas Frequentes": "D├║vidas sobre rastreio? Acesse 'Meus Pedidos'. Problemas com o pagamento? Fale conosco via WhatsApp. Trocas? Consulte nossa pol├¡tica de devolu├º├úo.",
    "Minha Conta": "Na sua conta voc├¬ pode gerenciar seus endere├ºos, verificar o status dos seus pedidos e baixar notas fiscais. Acesse com seu e-mail de cadastro.",
    "Meus Pedidos": "Acompanhe cada etapa do seu pedido em tempo real. Desde a separa├º├úo no estoque at├® a entrega na sua porta. O c├│digo de rastreio fica dispon├¡vel logo ap├│s o envio.",
    "Central de Atendimento": "Nosso time est├í pronto para ajudar. Atendimento via WhatsApp, E-mail ou formul├írio de contato de segunda a sexta, das 08h30 ├ás 18h00.",
    "Fale Conosco": "Precisa de algo urgente? O bot├úo de WhatsApp no canto inferior direito ├® o caminho mais r├ípido. Se preferir, use o formul├írio de contato acima."
  };
    ...
    const fetchData = async () => {
      try {
        ...
        if (isCatalog) {
          ...
          if (error || !data) throw new Error('Cat├ílogo n├úo encontrado');
          ...
          if (error || !data) throw new Error('Loja n├úo encontrada');
  ...
  const handleNavClick = (section: string) => {
    if (section === 'In├¡cio' || section === 'Loja') {
      setSearchQuery('');
        ...
        <div className={cn("py-2 text-[11px] text-white flex justify-between items-center", topBarBg)}>
          <div className="max-w-7xl mx-auto px-4 flex justify-between items-center w-full">
            ...
            <div className="flex items-center gap-6 opacity-90 hidden md:flex">
              ...
              <a
                href={store.whatsapp ? `https://wa.me/${store.whatsapp.replace(/\D/g, '')}?text=Ol├í! Preciso de ajuda.` : '#'}
                target="_blank"
          ...
          <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-[52px]">
            <div className="flex items-center gap-8 h-full">
              ...
              <nav className="flex items-center gap-6 h-full">
                {['In├¡cio', 'Loja', 'Categorias', 'Produtos', 'Contato'].map((item, i) => (
    ...
    return (
      <div key={product.id} className="bg-white border text-center border-gray-100 rounded-sm hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(0,0,0,0.06)] transition-all flex flex-col items-center p-5 relative group cursor-pointer" onClick={() => setSelectedProduct(product)}>
        ...
        <div className="w-full text-left space-y-1">
          <p className="text-[10px] font-medium text-gray-400 capitalize">{categories.find(c => c.id === product.category_id)?.name || 'Eletr├┤nicos'}</p>
      ...
      <section className="py-10 bg-white border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-4">
          <div className="border border-gray-100 rounded-sm grid grid-cols-2 lg:grid-cols-4 divide-x divide-y lg:divide-y-0 divide-gray-100 p-6 md:p-8">
            {[
              { icon: Truck, title: "Frete Gr├ítis", subtitle: "Para todos os pedidos acima de R$ 100" },
              { icon: ShieldCheck, title: "Devolu├º├úo em 30 Dias", subtitle: "Para trocas de produtos" },
              { icon: CreditCard, title: "Pagamento Seguro", subtitle: "Cart├Áes de pagamento aceitos" },
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-500 leading-relaxed">{store.description || "Sua melhor escolha com entrega r├ípida e garantia total. Produtos que facilitam o seu dia a dia."}</p>
              <div className="flex items-center gap-2">
                {store.facebook_url && (
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-2 text-xs text-gray-500 font-medium">
                ...
                <li className="hover:text-[var(--theme-primary)] cursor-pointer">Trocas e Devolu├º├Áes</li>
                <li className="hover:text-[var(--theme-primary)] cursor-pointer">Informa├º├Áes de Envio</li>
                <li className="hover:text-[var(--theme-primary)] cursor-pointer">Rastreie seu Pedido</li>
                <li className="hover:text-[var(--theme-primary)] cursor-pointer">Pol├¡ticas de Privacidade</li>
              </ul>
            </div>
      ...
      >
        ...
        <div className="p-4 md:p-5 space-y-2">
          <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest leading-none">
            {categories.find(c => c.id === prod.category_id)?.name || 'Cole├º├úo'}
          </p>
          <h3 className={cn(
        ...
        <div className="bg-[#f0f4ef] py-20 rounded-b-[4rem] mx-4 overflow-hidden relative">
          <div className="max-w-7xl mx-auto px-10 grid md:grid-cols-2 items-center gap-12">
            <div className="space-y-6">
              ...
              <p className="text-gray-500 font-medium">Produtos sustent├íveis e naturais para seu bem estar.</p>
        ...
        <div className="bg-[#f5ebe0] py-24 border-b border-[#e3d5ca]">
          <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center gap-10">
            <div className="flex-1 space-y-6">
              ...
              <p className="text-[#7f6a5b] font-medium text-lg">Descubra nossa cole├º├úo ├║nica de produtos selecionados.</p>
            </div>
      ...
      return (
        <div className="bg-white py-20 border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-2 items-center gap-12">
            ...
            <div className="space-y-6">
              <span className="text-sm font-bold text-blue-600 tracking-wider uppercase">Solu├º├Áes que entregam</span>
              <h2 className="text-4xl md:text-5xl font-black text-gray-900 tracking-tight leading-tight">Excel├¬ncia e<br />Confiabilidade.</h2>
              <p className="text-gray-600 text-lg">Cat├ílogo premium pensado para resultados pr├íticos e duradouros.</p>
              <button className={getButtonStyle("bg-blue-600 text-white px-8 py-3 rounded text-sm hover:bg-blue-700")}>Ver Cat├ílogo</button>
            </div>
          </div>
        </div>
      );
        ...
        <div className="bg-[#faf9f6] py-32 border-b border-[#e6e2d3]">
          <div className="max-w-4xl mx-auto px-4 text-center space-y-10">
            <h2 className="text-3xl md:text-5xl font-light text-[#8b8577] tracking-widest leading-relaxed">
              Equil├¡brio em cada <br /><span className="font-medium text-[#6b655b]">detalhe</span>.
            </h2>
          ...
          <div className="max-w-7xl mx-auto px-4 flex flex-col items-center">
            <div className="border-4 border-[#3e2723] p-2 bg-[#ffc107] shadow-[8px_8px_0_0_#3e2723] mb-12 -rotate-1">
              <h2 className="text-4xl md:text-6xl font-black text-[#3e2723] uppercase tracking-tighter px-4 py-2 bg-[#F3E5AB] border-2 border-[#3e2723]">
                Estilo <span className="text-[#d84315]">Retr├┤</span>
              </h2>
            </div>
      ...
      <div className="bg-gray-50/50 py-16 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            { icon: <Truck className="text-indigo-500" />, title: "Entrega R├ípida", desc: "Para todo o Brasil" },
            { icon: <CreditCard className="text-orange-500" />, title: "Parcelamento", desc: "Em at├® 12x sem juros" },
        ...
        <div className="sticky top-0 z-50 flex flex-col w-full shadow-md">
          ...
          <div className="max-w-7xl mx-auto px-4 flex flex-wrap md:flex-nowrap items-center justify-between md:gap-8 gap-y-3">
            ...
            <div className="w-full md:flex-1 order-last md:order-none relative group max-w-2xl">
              <input
                type="text"
                placeholder="O que voc├¬ est├í procurando?"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              ...
              >
                ...
                <div className="hidden lg:flex flex-col">
                  <span className="text-[10px] font-bold text-gray-500 uppercase tracking-tighter">
                    {customerSession ? 'Ol├í, Cliente' : 'Minha Conta'}
                  </span>
  ...
  const handleWhatsAppAlert = (product: any) => {
    ...
    if (!phone) {
      showNotification('WhatsApp da loja n├úo configurado.', 'error');
      return;
    }
    const skuInfo = product.sku ? ` (SKU: ${product.sku})` : '';
    const message = `Ol├í, tenho interesse no produto: ${product.name}${skuInfo}, mas vi que est├í indispon├¡vel no momento. Poderia me avisar quando voltar ao estoque?`;
  ...
  const handleDirectPurchase = async (price: number, method: 'pix' | 'card') => {
    ...
    if (!isPickup) {
      if (!checkoutAddress.fullName || !checkoutAddress.cep || !checkoutAddress.number) {
        showNotification('Preencha os campos obrigat├│rios de endere├ºo (Nome, CEP, N├║mero).', 'error');
        return;
      }
      if (shippingOptions.length > 0 && !selectedShipping) {
        showNotification('Por favor, selecione uma op├º├úo de frete para continuar.', 'error');
        return;
      }
    }
    ...
    const shippingMethodToSave = isPickup ? { id: 'pickup', name: 'Retirar em M├úos', price: 0 } : selectedShipping;

    if (!customerSession?.user) {
      showNotification('Voc├¬ precisa estar logado para comprar.', 'info');
      setIsAuthModalOpen(true);
      return;
    }
    ...
    const addressParts = [
      ...
      checkoutAddress.number ? `N┬║: ${checkoutAddress.number}` : '',
    ...
    try {
      const { data, error } = await supabase.from('orders').insert([{
        ...
        address: isPickup ? 'Retirar em M├úos' : addressParts,
      ...
      showNotification(`Erro ao criar pedido: ${err.message || 'Verifique sua conex├úo.'}`, 'error');
      return; 
    }
    ...
    if (isPickup) {
      ...
      const methodStr = method === 'pix' ? 'no PIX' : 'no Cart├úo';
      const message = `Ol├í! Quero comprar o produto: ${selectedProduct.name}${skuInfo}${varInfo} por ${(finalPriceWithShipping).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} ${methodStr}.\n\nModalidade: Ô£ï RETIRAR EM M├âOS`;
      window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`, '_blank');
      return;
    }
    ...
    try {
      ...
      if (data?.preference_id) {
        ...
        throw new Error('Link de pagamento n├úo retornado');
      }
    } catch (mpError: any) {
      console.log('Mercado Pago connect error or not configured:', mpError);
      showNotification('Pagamento online indispon├¡vel. A loja precisa configurar o Mercado Pago.', 'error');
    }
  };
                ...
                <div className="space-y-4 pb-4">
                    ...
                    <div className="space-y-4">
                      {paymentMethod === 'pix' ? (
                        <div className="flex items-center gap-4 p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-black text-emerald-600/60 uppercase tracking-widest">├Ç vista com {Number(selectedProduct.pix_discount_percent || 10)}% de desconto adicional</p>
                          </div>
                        </div>
                      ) : (
                        <div className="flex items-center gap-4 p-4 bg-orange-50 rounded-2xl border border-orange-100">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-black text-orange-600 uppercase tracking-widest">Parcelamento sem juros no cart├úo</p>
                          </div>
                        </div>
                      )}
                      ...
                      {(selectedVariation?.estoque ?? selectedProduct.estoque) > 0 && (selectedVariation?.estoque ?? selectedProduct.estoque) <= 5 && (
                        <div className="mt-2 p-2 bg-orange-100 text-orange-700 text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-2 animate-pulse">
                          ...
                          ├Ültimas {(selectedVariation?.estoque ?? selectedProduct.estoque)} unidades dispon├¡veis!
                        </div>
                      )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest underline decoration-[#f70]/30 underline-offset-4">56 avalia├º├Áes</span>
                      <div className="w-1 h-1 bg-gray-200 rounded-full" />
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">C├│d: {selectedVariation?.sku || selectedProduct.sku || '---'}</span>
                    </div>
                  </div>
                  ...
                  {selectedProduct.product_variations?.length > 0 && (
                    <div className="space-y-4">
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Selecione a Varia├º├úo</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {selectedProduct.extra_info.technical && (
                        <div className="space-y-1 p-3 bg-gray-50 rounded-2xl border border-gray-100">
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">Especifica├º├Áes</p>
                      ...
                      {selectedProduct.extra_info.informative && (
                        <div className="space-y-1 p-3 bg-indigo-50/30 rounded-2xl border border-indigo-100/50">
                          <p className="text-[9px] font-black text-indigo-400 uppercase tracking-[0.2em]">Informa├º├Áes</p>
                    ...
                    <div className="border border-gray-100 rounded-3xl p-1 overflow-hidden bg-white/50">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[11px] font-black uppercase tracking-[0.2em] text-gray-900">Descri├º├úo do Produto</span>
                        </div>
                        <ChevronDown
                          size={18}
                          className={cn(
                            "text-gray-400 transition-transform duration-500",
                            showDescri├º├úo ? "rotate-180 text-[#f70]" : ""
                          )}
                        />
                      </button>
                      <AnimatePresence>
                  ...
                  <div className="p-3 rounded-[1.5rem] bg-gray-50/50 border-2 border-dashed border-gray-200/60">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        ...
                        <p className={cn("text-[10px] font-black uppercase tracking-widest", paymentMethod === 'card' ? 'text-gray-900' : 'text-gray-400')}>Cart├úo</p>
                        {paymentMethod === 'card' && (
                      ...
                      >
                        ...
                        {paymentMethod === 'pix' && (
                          <div className="absolute -bottom-1 -right-1 px-2 py-1 bg-emerald-500 text-white text-[8px] font-black rounded-tl-xl uppercase tracking-tighter">Melhor Op├º├úo</div>
                        )}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 border-t border-dashed border-gray-100 pt-3 space-y-3">
                    ...
                    {canShip ? (
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setSelectedShipping({ id: 'pickup', name: 'Retirar em M├úos', price: 0 });
                              setShippingOptions([]);
                            }}
                            className={cn(
                              "p-3 rounded-2xl border-2 text-left transition-all duration-200 relative overflow-hidden",
                          ...
                          >
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm">Ô£ï</span>
                            ...
                            <p className="text-[10px] font-bold text-gray-500">Retirar em m├úos</p>
                      ...
                      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-2xl flex items-start gap-3">
                        <span className="text-xl shrink-0">ÔÜá´©Å</span>
                        <div>
                          <p className="text-[10px] font-black text-yellow-700 uppercase tracking-widest">Produto indispon├¡vel para envio</p>
                          <p className="text-[10px] font-bold text-yellow-600 mt-0.5">Apenas retirada em m├úos dispon├¡vel.</p>
                        </div>
                      </div>
                    )}
                    ...
                    {currentDeliveryMode === 'pickup' && (
                      <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-start gap-3">
                        <span className="text-xl shrink-0">Ô£ï</span>
                        <div>
                          <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Retirada gr├ítis!</p>
                          <p className="text-[10px] font-bold text-emerald-600 mt-0.5">Combine com o vendedor o local e hor├írio de retirada via WhatsApp.</p>
                          {store?.address && <p className="text-[10px] font-bold text-emerald-600 mt-1">­ƒôì {store.address}</p>}
                        </div>
                      </div>
                    )}
                    ...
                    {currentDeliveryMode === 'shipping' && (
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1">
                          <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">N├║mero *</label>
                          <input
                            type="text"
                            required
                    ...
                    {currentDeliveryMode === 'shipping' && !shippingLoading && shippingOptions.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Op├º├úo de Frete</p>
                          ...
                          >
                            ...
                            <div className="text-right">
                              <p className="text-xs font-black text-gray-900">
                                {option.price === 0 ? 'Gr├ítis' : `${(option.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`}
                              </p>
                    ...
                    {(selectedShipping || currentDeliveryMode === 'pickup') && (
                      <div className="p-3 bg-gray-50 rounded-xl border border-gray-100 space-y-1.5">
                        ...
                        <div className="flex justify-between text-[10px] font-bold text-gray-500">
                          <span>{currentDeliveryMode === 'pickup' ? 'Ô£ï Retirada em M├úos' : `Frete (${selectedShipping?.name})`}</span>
                          <span className={currentDeliveryMode === 'pickup' ? 'text-emerald-600 font-black' : ''}>
                            {currentDeliveryMode === 'pickup' ? 'Gr├ítis' : (selectedShipping?.price === 0 ? 'Gr├ítis' : `${(selectedShipping?.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`)}
                          </span>
                        </div>
                      ...
                      {brickLoading && (
                        <div className="flex flex-col items-center gap-3">
                          ...
                          {preferenceId && (
                            ...
                            >
                              N├úo carregou? Clique aqui para pagar
                            </button>
                          )}
                        </div>
                      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center space-y-6 md:space-y-8">
              ...
              {selectedProduct.product_variations?.length > 0 && (
                <div className="space-y-3">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Op├º├Áes</p>
              ...
              <div className="p-5 bg-gray-50 rounded-2xl flex items-center justify-between border border-gray-100">
                <div>
                  ...
                  {(selectedVariation?.estoque ?? selectedProduct.estoque) <= 5 && (selectedVariation?.estoque ?? selectedProduct.estoque) > 0 && (
                    <p className="text-[9px] font-black text-orange-500 uppercase mt-2 animate-pulse">├Ültimas {(selectedVariation?.estoque ?? selectedProduct.estoque)} unidades!</p>
                  )}
                  {(selectedVariation?.estoque ?? selectedProduct.estoque) === 0 && (
                    <p className="text-[9px] font-black text-red-500 uppercase mt-2">Produto indispon├¡vel</p>
                  )}
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <button onClick={() => setPaymentMethod('card')} className={cn("px-4 py-2 text-[10px] font-black uppercase rounded-lg border", paymentMethod === 'card' ? 'border-[#f70] bg-orange-50 text-[#f70]' : 'border-gray-200 text-gray-400 hover:border-gray-300')}>Cart├úo</button>
                </div>
              </div>
            ...
            <div className="w-full md:w-1/2 flex flex-col bg-[#0b0b0b] md:h-full relative overflow-visible md:overflow-hidden">
              <div className="w-full md:flex-1 md:overflow-y-auto p-6 md:p-8 custom-scrollbar relative space-y-6">
                ...
                <div className="md:sticky md:top-0 z-30 -mx-1 px-1 py-4 bg-[#0b0b0b] md:bg-[#0b0b0b]/90 md:backdrop-blur-md border-b border-white/5 mb-4">
                  ...
                  {(selectedVariation?.estoque ?? selectedProduct.estoque) === 0 && (
                    <p className="text-[10px] font-black text-red-500 uppercase tracking-widest mt-2">Indispon├¡vel</p>
                  )}
                  {(selectedVariation?.estoque ?? selectedProduct.estoque) > 0 && (selectedVariation?.estoque ?? selectedProduct.estoque) <= 5 && (
                    <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest mt-2 animate-pulse">├Ültimas {(selectedVariation?.estoque ?? selectedProduct.estoque)} unidades</p>
                  )}
                </div>
              </div>
            ...
            <div className="w-full md:w-1/2 flex flex-col bg-slate-900/50 backdrop-blur-sm md:h-full relative overflow-visible md:overflow-hidden">
                ...
                <div className="space-y-6">
                  ...
                  <div className="md:sticky md:top-0 z-30 -mx-1 px-1 py-4 bg-[#020617] md:bg-[#020617]/90 md:backdrop-blur-md rounded-2xl border border-blue-500/20 mb-4">
                    ...
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="text-blue-400 text-xs font-black uppercase leading-none">Subir N├¡vel</span>
                    </div>
                    {(selectedVariation?.estoque ?? selectedProduct.estoque) === 0 && (
              ...
              <div className="mt-8 space-y-3 px-6 md:px-8 pb-8">
                ...
                <div className="flex items-center justify-center gap-4 text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] pt-2">
                  ...
                  <span className="flex items-center gap-1"><Truck size={12} /> Entrega R├ípida</span>
                </div>
              </div>
            </div>
          </motion.div>
              ...
              <div className="w-full md:w-1/2 p-6 md:p-8 space-y-6">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-[10px] font-bold text-gray-400">4.9 (128 vendas este m├¬s)</span>
                  </div>
                  {selectedProduct.description && (
                ...
                {selectedProduct.product_variations?.length > 0 && (
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Escolha uma op├º├úo</p>
                    <div className="flex flex-wrap gap-2">
                      {selectedProduct.product_variations.map((v: any) => (
                        <button
                          key={v.id}
                ...
                <div className="p-6 bg-red-50 rounded-2xl border border-red-100 flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-black text-red-600 uppercase mb-1">Pre├ºo Promocional</p>
                    <p className="text-3xl font-black text-gray-900">{(finalPrice).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-black uppercase">
                    {(selectedVariation?.estoque ?? selectedProduct.estoque) === 0 ? (
                      <span className="text-red-600 animate-pulse">Produto indispon├¡vel!</span>
                    ) : (
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={() => setPaymentMethod('card')} className={cn("flex-1 p-4 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all text-center", paymentMethod === 'card' ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-gray-200')}>
                    <span className="text-[10px] font-black uppercase tracking-widest">Cart├úo</span>
        ...
        >
          <div className="bg-[#1a1a1a]/80 backdrop-blur-sm border border-white/10 p-4 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-0.5">Notifica├º├úo</p>
        ...
        <div className="max-w-md w-full text-center space-y-8 bg-white/5 p-12 rounded-[3rem] border border-white/10 backdrop-blur-sm">
          ...
          <div className="space-y-2">
            ...
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em] leading-relaxed">
              {error || "A loja que voc├¬ procura n├úo existe ou foi desativada no momento."}
            </p>
          </div>
          <button
      ...
      <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
        <div className="max-w-md mx-auto min-h-screen bg-white shadow-2xl relative">
          <div className="p-8 text-center" style={{ backgroundColor: (store.theme_color || '#5551FF') + '15' }}>
            ...
            <h1 className="text-2xl font-black text-gray-900">{store.name || 'Meu Cat├ílogo'}</h1>
          ...
          <div className="p-6">
            ...
            {products.length === 0 ? (
              <div className="py-12 text-center text-gray-400">
                ...
                <p className="font-bold text-xs uppercase tracking-widest">Nenhum produto dispon├¡vel</p>
              </div>
            ) : (
              ...
              >
                ...
                <div className="p-6 overflow-y-auto flex-1 bg-white">
                  ...
                  {catalogProductDetails.description && (
                    <div className="mb-6">
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Descri├º├úo</p>
                ...
                <div className="p-4 bg-white border-t border-gray-100 shrink-0">
                  <a
                    href={`https://wa.me/${store.whatsapp?.replace(/\D/g, '') || ''}?text=Ol├í,%20gostaria%20de%20comprar%20o%20produto:%20${encodeURIComponent(catalogProductDetails.name)}%20(SKU:%20${encodeURIComponent(catalogProductDetails.sku || '')})`}
                    target="_blank"
                    rel="noreferrer"
  ...
  return (
    ...
    >
      ...
      {/* Cart Drawer ÔÇö Multi-Step Checkout */}
      <AnimatePresence>
        {isCartOpen && (() => {
          ...
          const buildWhatsAppMessage = () => {
            const itemsList = cart.map((item: any) => {
              ...
              return `ÔÇó ${item.quantity}x ${item.name}${skuStr}${variation} ÔÇö ${(Number(item.price) * item.quantity).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
            }).join('\n');
            const shippingLabel = selectedShipping ? `­ƒôª *Frete (${selectedShipping.name}):*` : `­ƒÜÜ *Frete (${cartShippingZone?.label || 'A organizar'}):*`;
            const shippingLine = (selectedShipping || cartShippingZone)
              ...
              : '­ƒÜÜ *Frete:* A consultar';
            const addressLine = cartDeliveryAddress.trim()
              ? `­ƒôì *Endere├ºo:* ${cartDeliveryAddress.trim()}${cartNeighborhood.trim() ? ` ÔÇö ${cartNeighborhood.trim()}` : ''}`
              : cartNeighborhood.trim() ? `­ƒôì *Bairro:* ${cartNeighborhood.trim()}` : '';
            return [
              `­ƒøÆ *Novo Pedido ÔÇö ${store.name}*`,
              '',
              '*Produtos:*',
              itemsList,
              '',
              `­ƒÆ░ *Subtotal:* ${cartTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`,
              shippingLine,
              `Ô£à *Total:* ${cartFinalTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`,
              `­ƒÆÜ *Total no Pix:* ${cartFinalPixTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`,
              '',
              `­ƒæñ *Cliente:* ${cartCustomerName.trim()}`,
              cartCustomerPhone.trim() ? `­ƒô▒ *Telefone:* ${cartCustomerPhone.trim()}` : '',
              cartCep.trim() ? `­ƒô« *CEP:* ${cartCep.trim()}` : '',
              addressLine,
            ].filter(Boolean).join('\n');
          };
          ...
          const saveOrderAndGetId = async (method: string): Promise<string> => {
            try {
              ...
              showNotification(`Erro ao salvar pedido: ${err.message || 'Verifique sua conex├úo e tente novamente.'}`, 'error');
            }
            return '';
          };
          ...
          const handleGoToCheckout = () => {
            if (!customerSession) {
              showNotification('Voc├¬ precisa estar logado para continuar com a compra.', 'info');
              setIsAuthModalOpen(true);
              return;
            }
          ...
          const handleCartWhatsApp = async () => {
            ...
            const phone = store.whatsapp?.replace(/\D/g, '');
            if (!phone) { showNotification('WhatsApp da loja n├úo configurado.', 'error'); return; }
          ...
          const handleCartPaymentConfirm = async () => {
            ...
            try {
              ...
              showNotification('Pagamento online indispon├¡vel. Usando WhatsApp como alternativa.', 'info');
              if (phone) {
                window.open(`https://wa.me/${phone}?text=${encodeURIComponent(buildWhatsAppMessage())}`, '_blank');
              }
              setCartCheckoutStep('success');
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                      {cart.length === 0 ? (
                        ...
                        <div className="p-5 space-y-5">
                          ...
                          {/* Shipping zones */}
                          <div className="space-y-2">
                            <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 flex items-center gap-1.5"><Truck size={11} /> Regi├úo de Entrega</p>
                            <div className="grid grid-cols-2 gap-2">
                            ...
                            <div className="space-y-2">
                              ...
                              {shippingOptions.length > 0 && (
                                <div className="space-y-2 py-1">
                                  <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Op├º├Áes de Frete</p>
                              ...
                              <input type="text" placeholder="Rua e n├║mero (opcional)" value={cartDeliveryAddress} onChange={e => setCartDeliveryAddress(e.target.value)}
                                className="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-xs font-bold text-gray-900 placeholder:text-gray-300 focus:border-[var(--theme-primary)] outline-none transition-colors bg-white" />
                            </div>
                          </div>
                          ...
                          <div className="rounded-2xl border border-gray-100 overflow-hidden">
                            ...
                            <div className="p-4 space-y-2">
                              ...
                              <div className="flex justify-between"><span className="text-[11px] font-bold text-gray-500">Frete</span><span className={cn("text-[12px] font-black", cartShippingZone ? "text-gray-900" : "text-gray-400")}>{cartShippingZone ? (cartShippingZone.price === 0 ? 'A combinar' : `${(cartShippingZone.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`) : 'ÔÇö'}</span></div>
                  ...
                  <>
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-5">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <p className={cn("text-[10px] font-bold", cartPaymentMethod === 'pix' ? 'text-emerald-600' : 'text-gray-400')}>{(cartFinalPixTotal).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} ÔÇö aprova├º├úo imediata</p>
                          </div>
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <p className={cn("text-[12px] font-black", cartPaymentMethod === 'card' ? 'text-[var(--theme-primary)]' : 'text-gray-900')}>Cart├úo de Cr├®dito</p>
                      ...
                      <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 space-y-1">
                        ...
                        <p className="text-[11px] font-bold text-gray-700">­ƒæñ {cartCustomerName}</p>
                        {cartCustomerPhone && <p className="text-[11px] font-bold text-gray-500">­ƒô▒ {cartCustomerPhone}</p>}
                        {(cartDeliveryAddress || cartNeighborhood || cartCep) && (
                          <div className="pt-1">
                            <p className="text-[11px] font-bold text-gray-500 flex items-start gap-1">
                              <span className="shrink-0 mt-0.5">­ƒôì</span>
                              <span>{[cartDeliveryAddress, cartNeighborhood, cartCep].filter(Boolean).join(' ÔÇö ')}</span>
                            </p>
                            {selectedShipping && (
                              <p className="text-[11px] font-black text-[var(--theme-primary)] flex items-center gap-1 mt-1">
                                <Truck size={12} /> {selectedShipping.name} ÔÇö {(selectedShipping.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                              </p>
                            )}
                          </div>
                        )}
                        {cartShippingZone && <p className="text-[11px] font-bold text-gray-500">­ƒÜÜ {cartShippingZone.label}</p>}
                        <button onClick={() => setCartCheckoutStep('cart')} className="text-[9px] font-black text-blue-400 hover:text-blue-600 uppercase tracking-widest mt-1 block">Editar dados Ôåù</button>
                      </div>
                    </div>
                    ...
                    <div className="p-5 border-t border-gray-100 bg-white shrink-0 space-y-2">
                      ...
                      >
                        {cartBrickLoading ? (
                          ...
                          <><CreditCard size={18} /> Pagar com Cart├úo</>
                        ) : (
                ...
                {cartCheckoutStep === 'success' && (
                  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-6">
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="w-full rounded-2xl border border-gray-100 p-4 text-left space-y-2 bg-gray-50">
                      ...
                      <div className="flex justify-between text-[11px]"><span className="font-bold text-gray-500">Forma de pagamento</span><span className="font-black text-gray-900 capitalize">{cartPaymentMethod === 'pix' ? 'Pix' : cartPaymentMethod === 'card' ? 'Cart├úo' : 'WhatsApp'}</span></div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="w-full space-y-3">
                      {customerSession?.user && (
                        ...
                        >
                          <Package size={16} /> Pedido Salvo ÔÇö Fechar
                        </button>
                      )}
                      <button
                        onClick={() => {
          ...
          <div className="border-t border-white/5 pt-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start mb-16">
              ...
              <div className="space-y-8 md:text-right">
                <h5 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#f70]">Seguran├ºa</h5>
            ...
            >
              ...
              <div className="p-10">
                ...
                <form onSubmit={handleAuth} className="space-y-4">
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1.5">Senha</label>
                    <input
                      type="password"
                      value={authPassword}
                  ...
                  <button
                    ...
                    className="w-full h-14 bg-[var(--theme-primary)] text-white font-black uppercase tracking-widest text-xs rounded-xl hover:brightness-90 transition-all disabled:opacity-60 flex items-center justify-center gap-2"
                  >
                ...
                <p className="text-center text-xs text-gray-400 mt-6 font-medium">
                  {authMode === 'login' ? 'N├úo tem conta?' : 'J├í tem conta?'}{' '}
                  <button
          ...
          <div className="fixed inset-0 z-[200] flex justify-end">
            ...
            >
              ...
              <div className="flex items-start justify-between px-6 pt-6 pb-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={async () => { await supabase.auth.signOut(); setShowOrders(false); showNotification('Sess├úo encerrada', 'info'); }}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-12 gap-4">
                      ...
                      <div className="col-span-8 space-y-1.5">
                        <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Endere├ºo (Rua)</p>
                        <input
                          type="text"
                      ...
                      <div className="col-span-4 space-y-1.5">
                        <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">N├║mero</p>
                        <input
                          type="text"
                    ...
                    <div className="bg-orange-50 rounded-2xl p-5 space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-[var(--theme-primary)]">Configura├º├Áes</p>
                      </div>
                      <p className="text-[10px] font-bold text-orange-600/80 uppercase tracking-wide leading-relaxed">
                        As prefer├¬ncias de e-mail e notifica├º├Áes est├úo ativadas para sua conta.
                      </p>
                      <button
                      ...
                      >
                        {authLoading ? 'Salvando...' : 'Alterar Prefer├¬ncias'}
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    {customerOrders.length === 0 ? (
                      <div className="text-center py-16 text-gray-300 space-y-4">
                        ...
                        <p className="text-xs text-gray-300">Seus pedidos aparecer├úo aqui ap├│s a confirma├º├úo</p>
                      </div>
                    ) : (
                          ...
                          <div key={order.id} className="border border-gray-100 rounded-2xl overflow-hidden hover:border-gray-200 transition-colors">
                            ...
                            >
                              ...
                              {order.tracking_code && (
                                <div className="bg-blue-50/50 rounded-xl p-3 flex items-center justify-between border border-blue-100/50">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                      <p className="text-[8px] font-black uppercase tracking-widest text-blue-400">C├│digo de Rastreio</p>
                              ...
                              <p className="text-[10px] text-[var(--theme-primary)] font-black text-right">
                                {isExpanded ? 'Ôû▓ Ocultar hist├│rico' : 'Ôû╝ Ver hist├│rico completo'}
                              </p>
                            </div>
                            ...
                            {isExpanded && (
                              <div className="border-t border-gray-100 bg-gray-50/50 px-5 py-4">
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">­ƒôì Rastreio do Pedido</p>
                                {logs.length === 0 ? (
                                  <p className="text-xs text-gray-400 italic">Nenhuma atualiza├º├úo dispon├¡vel ainda.</p>
                                ) : (
                  ...
                  <div className="space-y-4">
                    {favorites.length === 0 ? (
                      <div className="text-center py-16 text-gray-300 space-y-4">
                        ...
                        <p className="text-xs text-gray-300">Clique no cora├º├úo nos produtos para salvar aqui</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-2 gap-4">
